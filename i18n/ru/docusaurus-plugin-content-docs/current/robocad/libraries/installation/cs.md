...
<Tabs
    ...
    <TabItem value="NuGet">
        Откройте файл *.csproj* (используйте версию, которая вам нужна, вместо 1.4.0):
        ```xml
        <ItemGroup>
            <PackageReference Include="RobocadCs" Version="1.4.0" />
        </ItemGroup>
        ```  
        Также для работы с изображением с камеры из симулятора или на реальном роботе следует вставить следующий блок в файл *.csproj*:
        ```xml
