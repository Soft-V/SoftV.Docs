...
The VMX Robotics Controller integrates a built‑in 9‑axis inertial measurement unit (IMU) based on navX‑technology, which provides comprehensive motion sensing and orientation tracking capabilities for robotics applications. However, only yaw is available for now.

The code snippet below sends yaw values to shufflecad.
